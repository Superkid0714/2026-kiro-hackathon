...
- **AI 백엔드 런타임**: Python 서버 애플리케이션 또는 AWS Lambda 호환 구조
- **AI 모델 호출**: AWS Bedrock
- **데이터 저장**: DynamoDB
...
- Bedrock 호출 코드는 `llm_client.py` 한 곳에만 둔다.
- `scoring.py`, `matching.py`는 외부 네트워크 호출과 Bedrock 관련 import를 포함하면 안 된다.
...
- 기본 구조는 `Main Backend + AI Backend + Bedrock + Shared DynamoDB`를 사용한다.
